...
class Ps_PaysonCheckout2ValidationModuleFrontController extends ModuleFrontController
{
    ...
    public function init()
    {
        ...
        Ps_PaysonCheckout2::paysonAddLog('Cart ID: ' . $cartId);
        ...
        if ($this->context->customer->isLogged() || $this->context->customer->is_guest) {
            Ps_PaysonCheckout2::paysonAddLog('Validation - customer is logged in.');
            ...
            if ((int) Customer::customerExists($checkout['customer']['email'], true, false) > 0) {
                Ps_PaysonCheckout2::paysonAddLog('Validation - load existing customer.');
        ...
        if ((int) $customer->newsletter !== 1 && $this->context->cookie->newsletter_sub == true) {
            ...
            $customer->update();
            Ps_PaysonCheckout2::paysonAddLog('Updated customer newsletter settting.');
        }
        ...
        Ps_PaysonCheckout2::paysonAddLog('$cart->id_address_delivery: ' . $cart->id_address_delivery);
        ...
        Ps_PaysonCheckout2::paysonAddLog('$new_delivery_options_serialized: ' . $new_delivery_options_serialized);
        Ps_PaysonCheckout2::paysonAddLog('Address ID: ' . $address->id);
        ...
        //Ps_PaysonCheckout2::paysonAddLog('Cart: ' . print_r($cart, true), 1, null, null, null, true);
        Ps_PaysonCheckout2::paysonAddLog('Checkout country: ' . $checkout['customer']['countryCode']);

        $checkoutTotal = $checkout['order']['totalPriceIncludingTax'];
        $cartTotal = $cart->getOrderTotal(true, Cart::BOTH);

        Ps_PaysonCheckout2::paysonAddLog('Checkout total: ' . $checkoutTotal);
        Ps_PaysonCheckout2::paysonAddLog('Cart total: ' . $cartTotal);
        ...
        if ($checkoutTotal !== $cartTotal) {
            ...
            Ps_PaysonCheckout2::paysonAddLog('Updated checkout to match cart.');
            Ps_PaysonCheckout2::paysonAddLog('Failed validation, reload.');
        ...
        Ps_PaysonCheckout2::paysonAddLog('Passed validation.');
        if (Tools::getIsset('validate_order')) {
