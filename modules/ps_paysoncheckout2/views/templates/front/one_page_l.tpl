...
{capture name=path}{l s='Checkout' mod='ps_paysoncheckout2'}{/capture}
...
<div class="payson-cf payson-main op-l-align">
    <div id="payson_cart_summary_wrapp">
        <div class="cart-grid-body col-xs-12 col-lg-8 left-col">
           
                <div class="card cart-container">
                    <div class="card-block">
                        <h1 class="h1">{l s='Shopping Cart' mod='ps_paysoncheckout2'}</h1>
                    </div>
                    <hr class="separator">
                ...
                {if isset($free_shipping_price_amount) AND $free_shipping_price_amount>0}
                    <div class="card cart-container">
                        <div class="card-block free-shipping">
                                {l s='Free shipping when you shop products for more than' mod='ps_paysoncheckout2'}&nbsp;<strong>{Tools::displayPrice($free_shipping_price_amount)}</strong>&nbsp;{l s='excl. shipping.' mod='ps_paysoncheckout2'}
                        </div>
                    </div>
                {/if}
                ...
                <div class="card card-payson-pay">
                    <div class="card-block">
                        <h1 class="h1">
                            {l s='Payment' mod='ps_paysoncheckout2'}
                        </h1>
                        {if isset($PAYSONCHECKOUT2_SHOW_OTHER_PAYMENTS) && $PAYSONCHECKOUT2_SHOW_OTHER_PAYMENTS}
                            <span class="wrap-alternative-methods">
                                <a href="{$link->getPageLink('order', true, null, 'step=1')|escape:'html':'UTF-8'}" class="alternative-methods" title="{l s='Other payment methods' mod='ps_paysoncheckout2'}">
                                    <span>{l s='Other payment methods' mod='ps_paysoncheckout2'}</span>
                                </a>
                            </span>
                        {/if}   
                    </div>
        ...
        <div class="cart-grid-body col-xs-12 col-lg-4 right-col">
            ...
            {if isset($controllername) && (isset($delivery_options) && $delivery_options|@count != 0) || isset($hookDisplayBeforeCarrier) || isset($hookDisplayAfterCarrier)}
                <div class="card payson-carrier-card">
                    <div class="card-block">
                        <h1 class="h1">
                            {l s='Carrier' mod='ps_paysoncheckout2'}
                        </h1>
                    </div>
                    <hr class="separator">
                    ...
                    {if (isset($delivery_options) && $delivery_options|@count != 0)}
                        <div class="card-block payson-carrier-card-block">
                            <form action="{$link->getModuleLink('ps_paysoncheckout2', $controllername, [], true)|escape:'html':'UTF-8'}" method="post" id="pcocarrier">
                                ...
                                <li class="payson-select-list__item li-delivery-option-{$carrier.id} {if $delivery_option == $carrier_id}selected{/if}">
                                    ...
                                    <label for="delivery_option_{$carrier.id}" class="payson-select-list__item__label">
                                        ...
                                        <span class="payson-select-list__item__nbr">
                                            ...
                                            {else}
                                                {l s='Free!' mod='ps_paysoncheckout2'}
                                            {/if}
                                        </span>
            ...
            {if isset($controllername)}
                <div class="card payson-message-card">
                    <form action="{$link->getModuleLink('ps_paysoncheckout2', $controllername, [], true)|escape:'html':'UTF-8'}" method="post" id="pcomessage">
                        <div class="card-block">
                            <h1 class="h1 payson-click-trigger {if !$message.message}payson-click-trigger--inactive{/if}">
                                {l s='Message' mod='ps_paysoncheckout2'}
                            </h1>
                        </div>
                        <hr class="separator">
                        <div class="pco-target" {if !$message.message}style="display: none;"{/if}>
                            <div class="card-block">
                                <p id="messagearea">
                                    <textarea id="message" name="message" class="payson-input payson-input--area payson-input--full" placeholder="{l s='Add additional information to your order (optional)' mod='ps_paysoncheckout2'}">{$message.message|escape:'htmlall':'UTF-8'}</textarea>
                                    <button type="button" name="savemessagebutton" id="savemessagebutton" class="btn btn-primary">
                                        <span>{l s='Save' mod='ps_paysoncheckout2'}</span>
                                    </button>
                                </p>
                            </div>
                        </div>
            ...
            {if isset($controllername) && $giftAllowed==1}
                <div class="card">
                    <form action="{$link->getModuleLink('ps_paysoncheckout2', $controllername, [], true)|escape:'html':'UTF-8'}" method="post" id="pcogift">
                        <div class="card-block">
                            <h1 class="h1 payson-click-trigger {if !$message.message}payson-click-trigger--inactive{/if}">
                                {l s='Gift-wrapping' mod='ps_paysoncheckout2'}
                            </h1>
                        </div>
                        <hr class="separator">
                        <div class="pco-target" {if $gift_message == '' && (!isset($gift) || $gift==0)}style="display: none;"{/if}>
                            <div class="card-block">
                                <p id="giftmessagearea_long">
                                    <textarea id="gift_message" name="gift_message" class="payson-input payson-input--area payson-input--full" placeholder="{l s='Gift message (optional)' mod='ps_paysoncheckout2'}">{$gift_message|escape:'htmlall':'UTF-8'}</textarea>
                                    ...
                                    <span class="payson-check-group fl-r full-width">
                                        ...
                                        <span id="giftwrappingextracost">{l s='Wrapping cost:' mod='ps_paysoncheckout2'} {Tools::displayPrice($gift_wrapping_price)}</span>
                                    </span>
